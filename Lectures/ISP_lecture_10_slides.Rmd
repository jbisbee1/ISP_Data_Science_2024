...
for(i in 1:100) {
  train <- fn %>% 
    sample_n(size = round(nrow(fn)*.8),replace = F)
  test <- fn %>% anti_join(train)
...
```{r}
wine <- read_rds(file = "https://github.com/jbisbee1/ISP_Data_Science_2024/raw/main/data/wine_quality_red.rds")
...
- RQ: What predicts the quality of wine?
...
- Let's create a categorical version of the quality variable

```{r}
wine <- wine %>%
  mutate(taste = ifelse(quality < 5,'Poor',
                        ifelse(quality > 6,'Tasty',
                               'Decent')))
wine %>%
  count(quality,taste)
```
...
```{r}
mRang <- ranger(taste~.,wine %>%
         mutate(taste = factor(taste,
                               levels = c('Poor','Decent','Tasty'))) %>%
         select(-quality),num.trees = 5000,mtry = 8)
```
...
mRang <- ranger(taste~.,wine %>%
         mutate(taste = factor(taste,
                               levels = c('Poor','Decent','Tasty'))) %>%
         select(-quality),num.trees = 5000,mtry = 8,probability = T)
toEval <- wine %>%
  mutate(prob_poor = mRang$predictions[,1],
         prob_decent = mRang$predictions[,2],
         prob_tasty = mRang$predictions[,3]) %>%
  mutate(truth_poor = factor((taste == 'Poor')+0,levels = c('1','0')),
         truth_decent = factor((taste == 'Decent')+0,levels = c('1','0')),
         truth_tasty = factor((taste == 'Tasty')+0,levels = c('1','0'))) %>%
  select(matches('prob|truth'))

roc_auc(toEval,truth_poor,prob_poor)
roc_auc(toEval,truth_decent,prob_decent)
roc_auc(toEval,truth_tasty,prob_tasty)
```
