...
--
  ...
  - $\beta$ is the amount $Y$ increases when $X$ increases by 1
...
1. `log(Y) ~ X`: 1 unit change in $X$ &rarr; $(exp(\beta)-1)*100$ % change in $Y$
  
2. `Y ~ log(X)`: 1% increase in $X$ &rarr; $\beta/100$ unit change in $Y$
  
3. `log(Y) ~ log(X)`: 1% increase in $X$ &rarr; $\beta$ % change in $Y$
...
--
  ...
  - Decimal, then 3 zeros, then the number
...
- Rule: take the number, and move the decimal $n$ places to the left, where $n$ is the negative number following the `e`
...
- Rule: take the number, and move the decimal $n$ places to the **right**, where $n$ is the positive number following the `e`

  - 74,700

---

# Practice: Categorical $Y$ = "Yes"
