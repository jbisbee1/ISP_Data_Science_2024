...
mv %>%
  ...
  pivot_longer(names_to = "metric",values_to = "dollars",cols = c("gross","budget")) %>%
...
mv %>%
  ...
  pivot_longer(names_to = "metric",values_to = "log_dollars",cols = c("gross_log","budget_log")) %>%
...
$$gross\_log = \underbrace{1.26}_{\alpha} + \underbrace{0.96}_{\beta} * budget\_log$$
...
--
  ...
  1. `log(Y) ~ X`: 1 unit change in $X$ &rarr; $(exp(\beta)-1)*100$ percent change in $Y$
  
  2. `Y ~ log(X)`: 1% increase in $X$ &rarr; $\beta/100$ unit change in $Y$
  
  3. `log(Y) ~ log(X)`: 1% increase in $X$ &rarr; $\beta$ percent change in $Y$
