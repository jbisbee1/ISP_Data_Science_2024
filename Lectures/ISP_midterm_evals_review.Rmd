...
- Overall, what did the open-ended questions reveal?
...
dtm %>%
  # filter(ResponseId == 'R_4hlIYsNIVSCtVGz')
  mutate(difficulty = grepl('hard|difficult|challeng',tolower(word)),
         speed = grepl('fast|slow|rush|time|too many',tolower(word)),
         easy = grepl("easy|good|expanding|delve",tolower(word))) %>%
  group_by(ResponseId) %>%
  summarise(difficult = mean(difficulty*n),
            speed = mean(speed*n),
            easy = mean(easy*n)) %>%
  # filter(easy == 1)
  gather(measure,value,-ResponseId) %>%
  ggplot(aes(x = value,
             y = reorder(measure,value))) + 
  geom_boxplot(outliers = T) + 
  labs(x = 'Average words',
       y = NULL,
       title = 'Distribution of evaluation words',
       subtitle = '3 Summary Dimensions')
```
...
- Cost: might not cover everything in the syllabus
...
- The teacher's paradox! Aim for the center to minimize dissatisfaction!

- 73% of the respondents talked about the speed, 68% talked about the difficulty, 18% talked about the ease
...
toanal3 <- toanal2 %>%
  ...
  ungroup() %>%
  mutate(difficulty = grepl('hard|difficult|challeng',tolower(response)),
         speed = grepl('fast|slow|rush|time|too many',tolower(response)),
         easy = grepl("easy|totally good|expanding|delve",tolower(response)))

toanal3 %>%
  summarise(difficulty = mean(difficulty),
            speed = mean(speed),
            easy = mean(easy)) %>%
  gather(dimension,proportion) %>%
  arrange(desc(proportion)) %>%
  left_join(toanal3 %>%
  summarise_at(vars(-ResponseId,-response),sum) %>%
  gather(dimension,nResp))
```
